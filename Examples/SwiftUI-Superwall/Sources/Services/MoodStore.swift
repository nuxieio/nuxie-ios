...
//
//  **SwiftUI Adaptation:**
//  This class is an ObservableObject, allowing SwiftUI views to automatically update
//  when mood data changes. Instead of NotificationCenter, we use @Published properties.
//
...
final class MoodStore: ObservableObject {
    ...
    /// All mood entries (keyed by date string)
    /// Published so SwiftUI views automatically update when data changes
    @Published private(set) var cache: [String: MoodEntry] = [:]
    ...
    func save(_ entry: MoodEntry) throws {
        ...
        // Update cache (will trigger @Published update and refresh UI)
    ...
    ///
    /// **Nuxie Integration Point:**
    /// This is a Pro feature - track "csv_export_gated" before calling this
    func exportCSV() -> Data? {
