...
final class FlowJourneyRunnerTests: AsyncSpec {
    override class func spec() {
        ...
        describe("FlowJourneyRunner") {
            ...
            it("falls back to paywall state when purchase action omits productId") {
                let flowId = "flow-implicit-paywall-purchase"
                let runtimeViewModel = ViewModel(
                    id: "vm-runtime",
                    name: "Runtime",
                    viewModelPathId: 0,
                    properties: [
                ...
                let runtimeInstance = ViewModelInstance(
                    viewModelId: "vm-runtime",
                    instanceId: "vmi-runtime",
                    name: "Paywall",
                    values: [
                        "paywall": AnyCodable([
                            "selectedProductId": "draft:paywall_main:0",
                            "selectedIndex": 0,
                        ] as [String: Any])
                    ]
                )
                let interaction = Interaction(
                    ...
                    actions: [
                        .purchase(
                            PurchaseAction(
                                placementIndex: AnyCodable(NSNull()),
                                productId: AnyCodable("")
                            )
                        )
                    ],
                ...
                let remoteFlow = makeRemoteFlow(
                    flowId: flowId,
                    interactionsByScreen: ["screen-1": [interaction]],
                    viewModels: [runtimeViewModel],
                    viewModelInstances: [runtimeInstance]
                )
                ...
                expect(controller.purchaseRequests.map(\.productId)).to(equal(["draft:paywall_main:0"]))
                expect(controller.purchaseRequests.first?.placementIndex as? Int).to(equal(0))
            }
